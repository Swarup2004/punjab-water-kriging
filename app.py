...
# new imports for colormap handling and normalization
...
# ----------------------------
# NEW: downstream ordering + differential computation
# ----------------------------
def _principal_axis_sort_indices(lat_series: pd.Series, lon_series: pd.Series):
    """
    Heuristic to order points along the main axis of variation (PCA / SVD on coordinates).
    Returns numpy array of indices sorted along the first principal axis.
    """
    coords = np.vstack([lat_series.values.astype(float), lon_series.values.astype(float)]).T
    # center
    mask = ~np.isnan(coords).any(axis=1)
    coords_valid = coords[mask]
    if coords_valid.shape[0] < 2:
        # fallback to original ordering
        return np.arange(len(lat_series))
    mean = coords_valid.mean(axis=0)
    centered = coords_valid - mean
    # SVD to get first principal direction
    try:
        ...
        sorted_order = idxs[np.argsort(proj)]
        return sorted_order
    except Exception:
        return np.arange(len(lat_series))

def compute_downstream_differential(df: pd.DataFrame, pollutant_series: pd.Series, signed: bool = True):
    """
    Given df (with lat/lon) and a pollutant_series (pd.Series indexed like df.index),
    compute downstream differential: for sites ordered along main axis, diff[i] = value[i] - value[i-1].
    Returns pd.Series aligned with df.index (NaN where missing coords).
    If signed==False, negative diffs are clipped to 0 (only new additions highlighted).
    """
    diff_series = pd.Series(index=df.index, dtype=float)
    if "lat" not in df.columns or "lon" not in df.columns:
        # cannot compute ordering
        return diff_series.fillna(0.0)

    sorted_idxs = _principal_axis_sort_indices(df["lat"], df["lon"])
    # If sorted_idxs doesn't cover all indices (due to NaNs), we handle them
    # Build ordered values list
    ordered_vals = []
    ordered_idx_list = []
    for i in sorted_idxs:
        if i in pollutant_series.index:
            ordered_vals.append(float(pollutant_series.loc[i]) if not pd.isna(pollutant_series.loc[i]) else np.nan)
            ordered_idx_list.append(i)
    # compute differences
    if not ordered_vals:
        return diff_series.fillna(0.0)
    ordered_arr = np.array(ordered_vals, dtype=float)
    # compute signed diff
    diffs = np.empty_like(ordered_arr)
    diffs[0] = ordered_arr[0]  # first gets its own value (or set to 0 if you prefer)
    for j in range(1, len(ordered_arr)):
        a = ordered_arr[j-1]
        b = ordered_arr[j]
        if np.isnan(a) or np.isnan(b):
            ...
            diffs[j] = b - a
    if not signed:
        # highlight only positives (new additions)
        diffs = np.where(np.isnan(diffs), np.nan, np.clip(diffs, 0, None))
    # map back
    for idx_val, idx in enumerate(ordered_idx_list):
        diff_series.loc[idx] = float(diffs[idx_val]) if not np.isnan(diffs[idx_val]) else np.nan
...
st.markdown("Click a marker to select a site. Use the controls to change pollutant encoding, months, and differential modes.")
...
# NEW: diverging colormap and diff controls
diverging_cmaps = ["RdBu", "RdBu_r", "coolwarm", "PiYG", "PRGn", "PuOr", "RdYlBu", "RdYlGn", "Spectral", "seismic"]
cmap_choice = st.sidebar.selectbox("Diverging colormap", diverging_cmaps, index=0)
use_differential = st.sidebar.checkbox("Color by downstream differential (current - upstream)", value=False)
differential_signed = st.sidebar.selectbox("Differential mode", ["signed", "positive-only"], index=0)
...
with map_col:
    st.subheader("Map")
    # Use OpenStreetMap base tiles (labels). Also add an alternative positron layer.
    m = folium.Map(location=MAP_CENTER, zoom_start=MAP_ZOOM, control_scale=True, tiles=None)
    # Add explicit tile layers so user sees OSM labels
    folium.TileLayer("OpenStreetMap", name="OpenStreetMap (labels)").add_to(m)
    ...
    # compute downstream differential series if requested
    if use_differential and pollutant and pollutant != "(none)":
        signed_flag = True if differential_signed == "signed" else False
        diff_series = compute_downstream_differential(df, agg_series, signed=signed_flag)
        color_series = diff_series
        color_label = f"{pollutant} (downstream diff)"
    else:
        color_series = agg_series
        color_label = f"{pollutant} (aggregate)"

    val_min = float(np.nanmin(color_series)) if len(color_series) > 0 else 0.0
    val_max = float(np.nanmax(color_series)) if len(color_series) > 0 else (val_min + 1.0)
    if val_min == val_max:
        # allow symmetric diverging mapping if centered around 0
        if val_min == 0.0:
            val_min, val_max = -1.0, 1.0
        else:
            val_max = val_min + 1.0

    # prepare matplotlib colormap sampling function
    try:
        cmap = mpl.cm.get_cmap(cmap_choice)
    except Exception:
        cmap = mpl.cm.get_cmap("RdBu")

    # If using differentials we want a diverging norm centered at 0
    if use_differential:
        # center at 0 for diverging maps
        center = 0.0
        # create TwoSlopeNorm to center color at zero
        try:
            norm = mpl.colors.TwoSlopeNorm(vmin=val_min, vcenter=center, vmax=val_max)
        except Exception:
            # fallback to linear norm
    ...
    def value_to_color_matplotlib(v):
        try:
            rgba = sm.to_rgba(float(v))
    ...
    for idx, row in filtered_df.iterrows():
        ...
        if pd.isna(lat) or pd.isna(lon):
            continue
        val = float(color_series.loc[idx]) if idx in color_series.index else 0.0
        color = value_to_color_matplotlib(val)
        ...
        marker = folium.CircleMarker(location=[lat, lon], radius=radius, color=color, fill=True, fill_color=color, fill_opacity=0.85,
    ...
    # legend (simple)
    legend_html = f"""
    <div style="position: fixed; bottom: 45px; left: 10px; z-index:9999; background-color: rgba(255,255,255,0.95); padding:8px; border-radius:6px;">
      <b>Legend</b><br>
      Mode: <b>{'Differential' if use_differential else 'Aggregate'}</b><br>
    ...
    if selected_idx is None:
        ...
        if selected_idx not in df.index:
            ...
            with st.expander("Small map / zoom"):
                sm = folium.Map(location=[row['lat'], row['lon']], zoom_start=13, tiles="OpenStreetMap")
